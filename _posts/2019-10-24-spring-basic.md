...
如何让事情变的简单一些。Spring 使用接口来降低耦合，并根据开发者的配置构建了组件依赖关系网络。而开发者需要的组件，常常由多个部分组成，Spring 根据预先配置好的关系网络匹配复杂组件所需要的基本组件并且装配好(context)，当开发者需要时可以直接向 Spring 索取(context.getBean)。
